...
```{r, echo=FALSE, cache=FALSE, results='hyde', warning=FALSE, comment=FALSE, warning=FALSE, include=FALSE}
library(readr)
dataset <- read.csv("last_withyears.csv", header=TRUE, row.names="X")
dataset <- head(dataset,-1)
summary(dataset)
...
x = c(rep(1,21), rep(2,21), rep(3,21), rep(4,21),rep(5,21), rep(6,21))
y=dataset[,1]
for (i in 2:6) {
  y=cbind(y,dataset[,i])
}
...
par(mar=rep(6,4))
boxplot(dataset[,1:6], las=2, col='red',ylim=c(0,30))
```

```{r, echo=FALSE, fig.show='hold', fig.height = 4, fig.width = 6, fig.align="center"}
matplot(dataset, las=2,type = "l", col="Red", xlab="Years: from 1994 to 2014", ylab="Number of sucicides per 100k")
```
...
print(loo_s)
plot(loo_separate, main ="PSIS diagnostic plot Separate Normal Model")
...
print(loo_h)
plot(loo_hierarchical,main ="PSIS diagnostic plot Hierarchical Normal Model")
...
plot(loo_model_s_nb, main ="PSIS diagnostic plot Separate Negative Bionomial Model")
...
plot(loo_model_h_nb, main ="PSIS diagnostic plot Hierarchical Negative Binomial Model")
...
samples_s_6 = extract(object=separate_fit6, permuted = TRUE, inc_warmup = FALSE, include = TRUE)
```

##8.1 Separate model

### SPECIFIC CONVERGENCE DIAGNOSTIC (HMC)
The following analysis of convergence diagnostic is reported just for the first group.The procedure and the results are similar for the others.First, the diagnostic plots are displayed.
...
```{r}
yreal <- read.csv("last_withyears.csv", header=TRUE, row.names="X")
yreal <- yreal[22,]
```
...
hist(vector2, main="15-24.years", ,xlim=c(7,15))
...
hist(vector3, main="25-34.years")
abline(v=c(mean(dataset[,3]),yreal[3]), col=c("red", "blue"), lwd=c(3, 3))
legend("topright", c("Prediction Mean", "Real data"), col=c("blue", "red"), lwd=3)

vector4=rep(0,4000)
for (i in 1:4000) {
  vector4[i]=mean(samples_s_4$y_state[i,])
}
hist(vector4, main="35-54.years")
...
hist(vector5, main="55-74.years")
abline(v=mean(dataset[,5]), col="red")
...
vector6=rep(0,4000)
for (i in 1:4000) {
  vector6[i]=mean(samples_s_6$y_state[i,])
}
six <- hist(vector6, main="Over 74 years")
abline(v=c(mean(dataset[,6]),yreal[6]), col=c("red", "blue"), lwd=c(3, 3))
legend("topright", c("Prior Mean", "Real data"), col=c("red", "blue"), lwd=3)
```
